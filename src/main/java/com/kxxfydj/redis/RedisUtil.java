...
//============================String=============================
    ...
    public <V> boolean setForBatch(List<String> keys, List<V> objectList) {
        RedisConnection redisConnection = redisTemplate.getConnectionFactory().getConnection();
        if (keys.size() == objectList.size()) {
            redisConnection.openPipeline();
            for (int i = 0; i < keys.size(); i++) {
                String key = keys.get(i);
                V object = objectList.get(i);
...
//================================Map=================================
     ...
     * @return 值
     */
    public <K, T, H> H hget(K key, T item) {
     ...
     * @return 对应的多个键值
     */
    public <K, H> Map<K, H> hmget(String key) {
     ...
     */
    public <H> boolean hmset(String key, Map<String, H> map) {
        try {
     ...
     * @return true成功 false失败
     */
    public <H> boolean hmset(String key, Map<String, H> map, long time) {
        try {
            redisTemplate.opsForHash().putAll(key, map);
            if (time > 0) {
                expire(key, time);
...
//===============================list=================================
     ...
     * @return
     */
    public <K, V> List<V> lGet(K key, long start, long end) {
        try {
     ...
     * @return
     */
    public <K, V> V lLeftPop(K key) {
        ListOperations<K, V> operations = redisTemplate.opsForList();
        return operations.leftPop(key);
    }

    public <K, V> V lRightPop(K key) {
        ListOperations<K, V> operations = redisTemplate.opsForList();
     ...
     * @return
     */
    public <K> long lGetListSize(K key) {
        try {
     ...
     * @return
     */
    public <K, V> V lGetIndex(K key, long index) {
        try {
            ListOperations<K, V> operations = redisTemplate.opsForList();
     ...
     * @return
     */
    public <K, V> boolean lSet(K key, V value) {
        try {
     ...
     * @return
     */
    public <V> boolean lSet(String key, V value, long time) {
        try {
            redisTemplate.opsForList().rightPush(key, value);
            if (time > 0) expire(key, time);
            return true;
     ...
     * @return
     */
    public <V> boolean lSet(String key, List<V> value) {
        try {
